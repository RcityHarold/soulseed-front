use serde_json::json;
use soulseed_agi_core_models::legacy::dialogue_event::{DialogueEvent, ToolInvocation, ToolResult};
use soulseed_agi_core_models::{
    AIId, AccessClass, AwarenessAnchor, AwarenessCycleId, AwarenessDegradationReason,
    AwarenessEvent, AwarenessEventType, AwarenessFork, ClarifyLimits, ClarifyPlan, ClarifyQuestion,
    ConversationScenario, CorrelationId, DecisionBudgetEstimate, DecisionExplain, DecisionPath,
    DecisionPlan, DecisionRationale, DialogueEventType, EnvelopeHead, EventId, HumanId, Provenance,
    SessionId, Snapshot, Subject, SubjectRef, TenantId, ToolPlan, ToolPlanBarrier, ToolPlanEdge,
    ToolPlanNode, TraceId,
};
...
use crate::services::dialogue::{
    build_message_event as build_runtime_message_event, MessageEventDraft,
};
...
pub fn make_dialogue_event_from_text(seq: u64, text: &str) -> DialogueEvent {
    let draft = MessageEventDraft {
        ...
        text,
        sequence_number: 200 + seq as u64,
        channel: Some("dialogue"),
        access_class: AccessClass::Internal,
        provenance: Some(Provenance {
            source: "fixture".to_string(),
            method: "sample_timeline".to_string(),
            model: None,
            content_digest_sha256: None,
        }),
        config_snapshot_hash: Some("fixture:v1"),
        config_snapshot_version: Some(1),
        timestamp_override_ms: None,
    };

    build_runtime_message_event(draft).expect("fixture dialogue event")
}
...
fn build_message_event(timestamp_ms: i64) -> DialogueEvent {
    let draft = MessageEventDraft {
        ...
        config_snapshot_hash: Some("fixture:v1"),
        config_snapshot_version: Some(1),
        timestamp_override_ms: Some(timestamp_ms),
    };

    build_runtime_message_event(draft).expect("fixture dialogue event (static)")
}
