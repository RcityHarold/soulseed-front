...
impl AppConfig {
    pub fn from_env() -> Self {
        ...
        if let Some(session) = read_env("SOULSEED_DEFAULT_SESSION") {
            config.default_session_id = Some(session);
        }
...
fn option_env_from_build(key: &str) -> Option<&'static str> {
    match key {
        ...
        "SOULSEED_DEFAULT_SESSION" => option_env!("SOULSEED_DEFAULT_SESSION"),
