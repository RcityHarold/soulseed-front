                ...
                match timeline_result {
                    Ok(env) => {
                        if let Some(payload) = env.data {
                            ...
                            let mut sessions_vec: Vec<WorkspaceSession> =
                                sessions.into_values().collect();
                            sessions_vec
                                .sort_by_key(|sess| sess.last_active_ms.unwrap_or_default());
                            ...
                            let pinned_sessions =
                                sessions_vec.iter().cloned().take(1).collect::<Vec<_>>();
                            ...
                            actions.set_workspace_error(Some("Workspace 接口返回空数据".into()));
                        }
                    }
                    Err(err) => {
                        tracing::error!("workspace timeline fetch failed: {err}");
                        actions
                            .set_workspace_error(Some(format!("无法加载 Workspace 数据: {err}")));
                    }
                }
            } else {
